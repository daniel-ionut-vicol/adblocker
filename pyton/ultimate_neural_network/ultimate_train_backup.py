        ...
        if os.path.isdir(class_path):  # Ensure it's a directory
            ...
            for root, _, files in os.walk(class_path):
                for image_file_name in files:
                    if image_file_name.endswith(".png") or image_file_name.endswith(
                        ".jpg"
                    ):
...
train_generator = Generator(
    train_paths, train_labels, BATCH_SIZE=config.BATCH_SIZE, is_training=True
)
val_generator = Generator(
    val_paths, val_labels, BATCH_SIZE=config.BATCH_SIZE, is_training=False
)
test_generator = Generator(
    test_paths, test_labels, BATCH_SIZE=config.BATCH_SIZE, is_training=False
)
