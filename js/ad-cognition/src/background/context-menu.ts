...
import { MESSAGE_TYPES, NOTIFIER_EVENTS } from 'Common/constants/common';
...
enum CONTEXT_MENU_ITEMS {
    ANALYZE_IMAGE = 'context_menu_analyse_image',
...
const analyzeImage = async (tab: Tab, url: string) => {
    log.info('Analyze image', url);
    tabUtils.sendMessageToTab(tab.id || 0, MESSAGE_TYPES.ANALYZE_IMAGE, { url });
};
...
const CONTEXT_MENU_MAP: any = {
    [CONTEXT_MENU_ITEMS.ANALYZE_IMAGE]: {
        action: analyzeImage,
        title: 'Analyze image',
        contexts: ['image'],
    },
...
const addMenuItem = async (key: CONTEXT_MENU_ITEMS) => {
    const { title, contexts } = CONTEXT_MENU_MAP[key];
...
const addContextMenu = async () => {
    await addMenuItem(CONTEXT_MENU_ITEMS.ANALYZE_IMAGE);
    await addSeparator();
...
const updateContextMenuThrottled = throttle(
    updateContextMenu,
...
const contextMenuClickHandler = (
    ...
    log.info('Click handler', info.srcUrl);
    if (contextMenu.title === 'Analyze image') {
        contextMenu.action(tab, info.srcUrl);
    }
...
const init = () => {
    ...
    notifier.addEventListener(
        [NOTIFIER_EVENTS.PROTECTION_UPDATED, NOTIFIER_EVENTS.SET_RULES],
        () => {
